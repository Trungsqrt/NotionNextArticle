...
/**
 * Resolves the previous and next sibling pages for a given page.
 * Sibling pages are defined as published pages that share the exact same
 * non-empty `parentTitle` (e.g. belonging to the same course or module).
 *
 * If the page is a top-level root database item (i.e. parentTitle is empty
 * or undefined), sibling pagination is not computed (returns { prevPage: null, nextPage: null }).
 *
 * Sibling pages are ordered by their `order` property ascending (0, 1, 2...),
 * falling back to title comparison when order is identical or null.
 *
 * @param {object} currentPage - Page object containing { id, slug, parentTitle }
...
export const getSiblingPages = cache(async function getSiblingPages(currentPage) {
  if (!currentPage || !currentPage.parentTitle || !currentPage.parentTitle.trim()) {
    return { prevPage: null, nextPage: null }
  }

  const allPosts = await getAllPosts()
  const targetParent = currentPage.parentTitle.trim().toLowerCase()

  // Filter all siblings with the same parentTitle
  const siblings = allPosts.filter(p => {
    return p.parentTitle && p.parentTitle.trim().toLowerCase() === targetParent
  })
  ...
  // Sort siblings by 'order' ascending (0, 1, 2...), fallback to title
  siblings.sort((a, b) => {
  ...
  const currentIndex = siblings.findIndex(
    s => s.id === currentPage.id || (currentPage.slug && s.slug === currentPage.slug)
  )
