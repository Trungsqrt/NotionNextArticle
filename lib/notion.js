...
renderer.addBlockRenderer(
  createBlockRenderer("bulleted_list_item", async (block) => {
    ...
    if (block.children?.length > 0) {
      childrenHtml = await renderer.render(...block.children)
    }
    return (
      `<ul class="list-disc list-outside ml-6 space-y-1 my-3 text-neutral-700 dark:text-neutral-300">` +
      `<li>${textHtml}${childrenHtml}</li>` +
      `</ul>`
    )
  })
)
...
renderer.addBlockRenderer(
  createBlockRenderer("numbered_list_item", async (block) => {
    ...
    return (
      `<ol class="list-decimal list-outside ml-6 space-y-1 my-3 text-neutral-700 dark:text-neutral-300">` +
      `<li>${textHtml}${childrenHtml}</li>` +
      `</ol>`
    )
  })
...
renderer.addBlockRenderer(
  createBlockRenderer("toggle", async (block) => {
    ...
    return (
      `<details class="my-3 rounded-lg bg-amber-50/60 dark:bg-neutral-800/60 border border-amber-200/50 dark:border-neutral-700 p-3 group">` +
      `<summary class="font-medium cursor-pointer select-none text-neutral-800 dark:text-neutral-200 flex items-center gap-2">` +
      `<span>${titleHtml}</span>` +
      `</summary>` +
      `<div class="mt-2 pl-4 border-l border-amber-300/60 dark:border-neutral-600 text-sm text-neutral-600 dark:text-neutral-300">` +
      `${childrenHtml}` +
      `</div>` +
      `</details>`
    )
