...
async function renderBlocks(blocks) {
  ...
  while (i < blocks.length) {
    ...
    if (block.type === "bulleted_list_item") {
      ...
      parts.push(`<ul class="list-disc list-outside pl-6 space-y-2 my-4 text-neutral-700 dark:text-neutral-300">${itemsHtml.join("")}</ul>`)
      continue
    }
    ...
    if (block.type === "numbered_list_item") {
      ...
      const itemsHtml = await Promise.all(group.map(b => renderer.render(b)))
      parts.push(`<ol class="list-decimal list-outside pl-6 space-y-2 my-4 text-neutral-700 dark:text-neutral-300">${itemsHtml.join("")}</ol>`)
      continue
    }
