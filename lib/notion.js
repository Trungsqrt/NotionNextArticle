...
function renderRichText(richTextArray) {
  ...
  return richTextArray.map(t => {
    ...
    if (t.annotations?.code) {
      content = `<code class="text-matcha-700 dark:text-matcha-300 bg-rice-paper-300/50 dark:bg-tea-slate-200/50 px-1.5 py-0.5 rounded text-sm">${content}</code>`
    }
    return content
  }).join('')
}
...
renderer.addBlockRenderer(
  createBlockRenderer("callout", async (block) => {
    ...
    return (
      `<div class="not-prose my-6 p-4 rounded-r-card border-l-4 border-matcha-500 bg-matcha-50/70 dark:bg-matcha-900/20 text-ink-700 dark:text-sage-200 flex items-start gap-3.5 shadow-zen-sm">` +
...
renderer.addBlockRenderer(
  createBlockRenderer("heading_1", async (block) => {
    ...
    return `<h1 class="text-2xl sm:text-3xl font-semibold text-neutral-800 dark:text-neutral-100 mt-8 mb-4 tracking-tight">${textHtml}</h1>`
  })
)
...
renderer.addBlockRenderer(
  createBlockRenderer("heading_2", async (block) => {
    ...
    return `<h2 class="text-xl sm:text-2xl font-medium text-neutral-800 dark:text-neutral-100 mt-6 mb-3">${textHtml}</h2>`
  })
)
...
renderer.addBlockRenderer(
  createBlockRenderer("heading_3", async (block) => {
    ...
    return `<h3 class="text-lg sm:text-xl font-medium text-neutral-700 dark:text-neutral-200 mt-4 mb-2">${textHtml}</h3>`
  })
)
...
renderer.addBlockRenderer(
  createBlockRenderer("image", async (block) => {
    ...
    return (
      ...
      `<div class="w-full overflow-hidden rounded-2xl shadow-sm border border-neutral-200/60 dark:border-neutral-800">` +
      `<img src="${escapeHtml(src)}" alt="${escapeHtml(captionHtml || '')}" class="w-full h-auto object-cover rounded-2xl block" loading="lazy" />` +
      `</div>` +
...
renderer.addBlockRenderer(
  createBlockRenderer("bulleted_list_item", async (block) => {
    const textHtml = renderRichText(block.bulleted_list_item?.rich_text)
    let childrenHtml = ""
    if (block.has_children && block.children?.length > 0) {
      childrenHtml = await renderBlocks(block.children)
    }
    return `<li class="pl-1">${textHtml}${childrenHtml}</li>`
  })
)

renderer.addBlockRenderer(
  createBlockRenderer("numbered_list_item", async (block) => {
    const textHtml = renderRichText(block.numbered_list_item?.rich_text)
    let childrenHtml = ""
    if (block.has_children && block.children?.length > 0) {
      childrenHtml = await renderBlocks(block.children)
    }
...
renderer.addBlockRenderer(
  createBlockRenderer("paragraph", async (block) => {
    ...
    if (!textHtml) return ""
    return `<p class="my-3 text-[0.98rem] leading-relaxed text-neutral-700 dark:text-neutral-300">${textHtml}</p>`
  })
)
...
renderer.addBlockRenderer(
  createBlockRenderer("quote", async (block) => {
    ...
    return `<blockquote class="my-4 border-l-4 border-matcha-500 pl-4 italic text-neutral-600 dark:text-neutral-300 bg-matcha-50/30 dark:bg-matcha-900/10 py-2.5 rounded-r">${textHtml}</blockquote>`
  })
)
...
async function renderBlocks(blocks) {
  ...
  while (i < blocks.length) {
    ...
    if (block.type === "bulleted_list_item") {
      ...
      parts.push(`<ul class="list-disc list-outside ml-6 space-y-1 marker:text-neutral-400 text-neutral-700 dark:text-neutral-300" style="list-style-type: disc !important;">${itemsHtml.join("")}</ul>`)
      continue
    }
    ...
    if (block.type === "numbered_list_item") {
      ...
      const itemsHtml = await Promise.all(group.map(b => renderer.render(b)))
      parts.push(`<ol class="list-decimal list-outside ml-6 space-y-1 marker:text-neutral-400 text-neutral-700 dark:text-neutral-300" style="list-style-type: decimal !important;">${itemsHtml.join("")}</ol>`)
      continue
    }
