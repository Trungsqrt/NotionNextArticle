 ...
 */
async function resolveMissingBlocks(recordMap, notion, depth = 0) {
  if (depth > 2) return recordMap // Max nested resolution depth
  ...
  if (missingIds.size > 0) {
    console.log(`[Notion] Explicitly fetching ${missingIds.size} missing child blocks (Depth: ${depth})...`)
    try {
      const fetchedBlocks = await notion.getBlocks(Array.from(missingIds))
      const blocksToAdd = fetchedBlocks.recordMap?.block || fetchedBlocks.block || {}
      Object.assign(recordMap.block, blocksToAdd)
      
      // Recursively resolve in case the newly fetched blocks have missing children
      await resolveMissingBlocks(recordMap, notion, depth + 1)
    } catch (err) {
      console.error("[Notion] Failed to fetch missing blocks:", err)
    }
  }
  return recordMap
}
