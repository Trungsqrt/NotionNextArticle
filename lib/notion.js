...
renderer.addBlockRenderer(
  createBlockRenderer("column_list", async (block) => {
    ...
    return `<div class="flex flex-col md:flex-row gap-6 md:gap-8 my-6 w-full items-start">${childrenHtml}</div>`
  })
)
...
renderer.addBlockRenderer(
  createBlockRenderer("code", async (block) => {
    ...
    return `<div class="not-prose my-6"><pre class="bg-neutral-800 dark:bg-neutral-900 text-neutral-100 p-4 rounded-xl overflow-x-auto my-6 font-mono text-sm leading-relaxed"><code class="font-mono text-sm leading-relaxed">${highlighted}</code></pre>${captionHtml ? `<div class="mt-2 text-center text-xs text-neutral-400 dark:text-neutral-500 italic">${captionHtml}</div>` : ""}</div>`
  })
)
