...
renderer.addBlockRenderer(
  createBlockRenderer("quote", async (block) => {
    ...
    return `<blockquote class="my-4 border-l-4 border-matcha-500 pl-4 pr-4 py-3 font-medium italic text-neutral-700 dark:text-neutral-200 bg-stone-50/80 dark:bg-stone-800/40 rounded-r-card">
      ${textHtml}
      ${childrenHtml ? `<div class="mt-3 not-italic font-normal">${childrenHtml}</div>` : ""}
    </blockquote>`
  })
)
